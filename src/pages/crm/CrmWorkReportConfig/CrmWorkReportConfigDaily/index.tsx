...
const dictionary =  (organizationId) => fetchFn(POSITION_API, {organizationId, reportType:1}).then(data=>data);
...
class CrmWorkReportConfigDaily extends React.Component<any,any>{
    ...
    async componentWillMount(){
        if(this.props.location.state && this.props.location.state.id){
            ...
            try {
                document.querySelectorAll('.ant-breadcrumb .ant-breadcrumb-link > span')[0].innerHTML = disabled ? '工作报告模板_查看丨日报' : '工作报告模板_编辑丨日报';
            } catch (error) {
            ...
            const entire = await getEntire()
            this.setState({entire: entire.data})
            const [
                // position,
                company,
