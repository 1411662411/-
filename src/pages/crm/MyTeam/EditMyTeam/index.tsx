...
const EditableNameCell = ({ editable, value, onChange }) => (
    <div>
        {editable
            ? <Input placeholder='请填写团队名称' style={{ margin: '-5px 0' }} value={value} maxLength={50} onChange={e => onChange(e.target.value.trim())} />
            : value
        }
    </div>
);
const EditableEnCell = ({ queryOrganizationsAndUsers, editable, value, onChange }) => (
    <div>
        {editable
            ? <Organizations
                needAll={false}
...
class EditMyTeam extends React.Component<any, any>{
    ...
    renderEnColumns(text, record, column) {
        return (
            <EditableEnCell
                queryOrganizationsAndUsers={this.state.specialSubordinateList}
                editable={record.editable}
                value={text}
                onChange={value => this.handleChange(value, record.id, column)}
            />
                ...
                <div className="editable-row-operations">
                    {
                        editable ?
                            ...
                            <span>
                                ...
                                <a onClick={() => this.setState({deleteTeamId: record.id, deleteTeamName: record.name},() => {
    ...
    render() {
        ...
        return (
            <div className="edit-crm-my-team">
                {/* <JoyoModal
                    title='删除团队'
                    width={420}
