...
import {
    ...
    exceptionModal,
    submitfetching,
    initPayment, //初始化请款单信息
    getPaymentListOfRequest, //获取收款方信息 收款单列表
...
class CashoutApproveSubmit extends React.Component<CashoutApproveSubmitProps, any> {
    ...
    constructor(props) {
        ...
        const initialState = {
            params: Immutable.fromJS({
                ...
                socialNature: 1, /* 社保请款性质 */
                ownStaffRequest: 0, /* 是否为内部员工*/
                recipientType: '', /* 收款方类型 */
                recipientName: '', /* 收款方名称 */
                deadline: '', /* 本次请款付款截止时间 */
    ...
    async componentWillMount() {
        ...
        /**
         * *init 初始化获取请款单信息 & 收款方信息中出款单列表
         */
        const data = await dispatch(initPayment(1));
        // console.log(data);
         ...
         * 获取 收款方信息中出款单列表
         */
        // let requestInfoId = null;
        // if(requestInfoId){
        //     dispatch(getPaymentListOfRequest({
        //         platform: 1,
        //         requestInfoId: null,
        //     }))
        // }
    ...
    approveSubmit = () => {
        ...
        const paymentJSON = this.information.state.dataSource.toJS();
        console.log(paymentJSON);
        let paymentJSONHasNull = false;
        paymentJSON.forEach(item => {
            if(!item.insuredStatus){
                message.error(`请选择 ${item.name} 的 参保类型`);
                paymentJSONHasNull = true ;
                return;
            }
            if(!item.insuredItem){
                message.error(`请选择 ${item.name} 的 对应收款项`);
                paymentJSONHasNull = true ;
                return;
            }
            if(!item.receiverName){
                message.error(`请选择 ${item.name} 的 收款方名称`);
                paymentJSONHasNull = true ;
                return;
            }
            if(!item.businessPlanPayTime){
                message.error(`请选择 ${item.name} 的 业务方计划付款截止日`);
                paymentJSONHasNull = true ;
                return;
            }
        });
        if(paymentJSONHasNull){
            return;
        }
        ...
        // debugger
        // console.log(params)
        dispatch(submitApprove({submit: params, paymentJSON: JSON.stringify([...paymentJSON])}));
    }
    clearData = (index) => { //清除数据
        const {
            requestInfoId,
        } = this.props;
        this.props.dispatch(deleteUploadRecord({ type: index,requestId: requestInfoId}));
    }
        ...
        return (
            <div key="cashoutApproveSubmit">
                ...
                <SocialInfo
                    ...
                    ownStaffRequest={ownStaffRequest} //是否为内部员工
                ...
                <Information 
                    paymentBillDataSource={paymentbillDataSource}
                    noDetailAmount={noDetailAmount}
                    ref={node => this.information = node}
                    isEdit
                    isSP
                    total={this.totalCalc()}
                    requestInfoId={this.props.requestInfoId}
                    updateParams={this.updateParams}
                    recipientType={recipientType}
                ...
                <Row type="flex" justify="center">
                    <Button onClick={() => console.log(this.information)}>保存</Button>
...
const mapStateToProps = (state:Any.Store, ownProps: TOwnProps):TStateProps => {
    ...
    return {
        ...
        requestInfoId: data.get('requestInfoId'),
        paymentList: data.get('paymentList'),
    }
};
