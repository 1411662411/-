...
export class OutputFile extends React.Component<OutputFileProps, any> {
                ...
                if (data.status === 0) {
                    ...
                    if (uuid) {
                        ...
                        instancePoll.start({ guid: uuid, ...pollData, })
                            .then((responeData) => {
                                instancePoll.poll({ ...pollData, topic: 2, guid: uuid }, (responeData) => responeData.data.loadingSwitch === false, (responeData) => {

                                    this.pollCallback(responeData, uuid);
                                });
                                // if (responeData.data.loadingSwitch === false && responeData.data.schedule == 100) {
                                //     this.pollCallback(responeData, uuid);
                                // }
                            }, (responeData) => {
    ...
    render() {
        ...
        return (<div {...this.outputProps() } style={{ display: 'inline-block' }}>
            {
