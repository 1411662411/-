...
import { isParent, hasErrors } from '../../../../util/crmUtil'
...
class FilterTableHeader extends React.Component<FilterTableHeaderProps, any>{
    constructor(props:FilterTableHeaderProps){
        super(props);
        this.state={
    ...
    render(){
        const { filtered, initialValue, value } = this.state;
        ...
        if(this.props.renderParent === true){
            popoverProps.getPopupContainer = (node) => node.parentNode as HTMLElement
        }

        return <div
            className='crm-custom-filter-table-thead-container'
        >
            {title}
            <Popover
                {...popoverProps}
            >
                <Icon onClick={this.filterOnClick} type="filter" style={{ color: reg ? '#fff' : '#108ee9', verticalAlign: 'initial' }} />
            </Popover>
        </div>
    }
}
...
class FilterTableHeaderItem extends React.Component<FilterTableHeaderItemProps,any>{
    ...
    render(){
        ...
        return <div
            className='crm-custom-filter-table-thead-filter'
